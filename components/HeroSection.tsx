...
export default function HeroSection({ profile }: { profile: Profile }) {
  return (
    <section
      id="home"
      aria-label="Bielcx"
      className="relative flex min-h-svh w-full flex-col justify-between"
    >
      {/* BARRA DE CONTEXTO */}
      <div // O pr extra abre espaço para o toggle de tema, que é `fixed` em
      // `top-6 right-6` e cairia em cima do `online`.
      className="flex items-center justify-between border-b border-line px-6 pr-14 py-5 text-xs tracking-[0.1em] text-ink-faint min-[900px]:px-10 min-[900px]:pr-[58px]">
      ...
      <div className="flex flex-1 flex-col items-start justify-center gap-12 px-6 py-14 min-[900px]:flex-row min-[900px]:items-center min-[900px]:justify-between min-[900px]:px-16">
        <div className="flex max-w-[660px] flex-col items-start gap-6">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-3">
            <a
              ...
              className="inline-flex items-center gap-2.5 rounded-full bg-brand-solid px-[26px] py-3.5 text-sm font-medium tracking-[0.04em] text-on-brand transition-colors hover:bg-[#b2622d] active:bg-[#8c491a]"
            >
              entrar em contato ↗
            </a>
            <a
        ...
        <div className="relative w-[240px] shrink-0 self-center overflow-hidden rounded-3xl border border-dashed border-[color:var(--frame-border)] bg-[linear-gradient(180deg,#14120f,#000)] h-[340px] min-[900px]:h-[420px] min-[900px]:w-[300px]">
          ...
          <span className="absolute left-4 top-3.5 size-[18px] border-l-2 border-t-2 border-brand-solid" />
          <span className="absolute right-4 top-3.5 size-[18px] border-r-2 border-t-2 border-brand-solid" />
          <span className="absolute bottom-3.5 left-4 size-[18px] border-b-2 border-l-2 border-brand-solid" />
          <span className="absolute bottom-3.5 right-4 size-[18px] border-b-2 border-r-2 border-brand-solid" />
      ...
      {/* BARRA DE STATUS — opaca de propósito: ela fecha a hero por baixo, e o
          fundo da página continua correndo atrás dela. */}
      <div className="flex items-center justify-between border-t border-line bg-surface px-6 py-[18px] text-[11px] tracking-[0.12em] text-ink-faint min-[900px]:px-10">
        <span className="flex gap-6 min-[900px]:gap-[26px]">
          {telemetry.map((item) => (
            <span
              key={item}
              className={item.startsWith("uptime") ? "hidden min-[900px]:inline" : undefined}
            >
              {item}
            </span>
          ))}
